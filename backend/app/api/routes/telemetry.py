...
async def fetch_and_predict_live_weather(
    lat: float = 26.7271, lon: float = 88.3953, db: Session = Depends(get_db)
):
    """Pulls live satellite data from Open-Meteo and predicts flood risk."""
    weather_data = fetch_open_meteo_live(lat, lon)
    if not weather_data or "current" not in weather_data:
        raise HTTPException(
            status_code=502, detail="Failed to fetch live weather from Open-Meteo"
        )
