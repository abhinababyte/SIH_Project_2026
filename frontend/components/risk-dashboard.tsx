...
import { SeverityBadge, severityColor, severityText } from "@/components/severity-badge";
import { Card } from "@/components/ui/card";
import { overallSeverity, SEVERITY_META, type Sensor, type Severity } from "@/lib/flood-data";
  ...
  return (
    <div className="relative flex size-36 items-center justify-center rounded-full bg-black/40 border border-white/5 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]">
      ...
      <svg viewBox="0 0 120 120" className="size-full -rotate-90 relative z-10">
        <circle cx="60" cy="60" r={r} className="fill-none stroke-white/5 stroke-[4]" />
        <circle
          cx="60"
          cy="60"
          r={r}
      ...
      <div className="absolute flex flex-col items-center justify-center z-10">
        ...
        <span className="text-[10px] text-white/50 linear-label mt-1">INDEX</span>
      </div>
    </div>
  );
}
...
export function RiskDashboard({ sensors }: { sensors: Sensor[] }) {
  ...
  const avg = sensors.reduce((sum, s) => sum + s.level, 0) / Math.max(1, sensors.length);
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 mb-2">
                <p className="linear-label linear-text-muted">CURRENT STATUS</p>
              </div>
              <SeverityBadge
                severity={overall}
                className="text-sm bg-white/5 border border-white/10"
                showDot={true}
              />
              <p className="max-w-56 text-sm linear-text-muted leading-relaxed mt-2 font-normal">
                {SEVERITY_META[overall].description}. Evaluated from {sensors.length} telemetry
                points.
              </p>
            </div>
          </div>
          ...
          return (
            <div key={s.id} className="linear-card p-5 group cursor-default">
              ...
              <div>
                ...
                <p className="linear-label linear-text-muted text-[10px]">{s.name}</p>
              </div>
