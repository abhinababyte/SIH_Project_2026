import { MapPin, Navigation, Search, Users, X } from "lucide-react";
  ...
  const routes = [
    {
      ...
      dest: "to Rasdale Community Hall",
      status: "Clear",
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {routes.map((r, i) => (
              <div
                key={i}
                className="flex items-center justify-between bg-white/[0.03] border border-white/5 rounded-2xl p-4 hover:bg-white/[0.05] transition-colors cursor-pointer"
              >
                <div className="flex items-start gap-3">
                  <div className={cn("size-2.5 rounded-full mt-1.5 shrink-0", r.color)} />
                  <div className="flex flex-col">
                    <span className="text-[15px] font-medium text-slate-200 leading-tight">
                      {r.name}
                    </span>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3">
            {shelters.map((s, i) => (
              <div
                key={i}
                className="flex flex-col bg-white/[0.03] border border-white/5 rounded-2xl p-4 hover:bg-white/[0.05] transition-colors cursor-pointer"
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span
                    className={cn("text-[10px] font-bold px-2 py-0.5 rounded-md", s.statusColor)}
                  >
                    {s.status}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-wrap gap-1.5">
                    {s.tags.map((t, j) => (
                      <span
                        key={j}
                        className="text-[10px] font-medium text-slate-300 bg-white/5 px-2 py-1 rounded-md"
                      >
                        {t}
                      </span>
                    ))}
