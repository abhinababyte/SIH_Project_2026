import { AlertTriangle, CloudRain, Droplets, Mountain, ShieldAlert, Waves, X } from "lucide-react";
...
export function WhatIfSimulator({ onClose }: { onClose?: () => void }) {
  ...
  const riskScore = Math.min(
    100,
    Math.round((rain / 200) * 40 + (soil / 100) * 20 + (river / 5) * 40),
  );
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#0a0f18] text-white">
      <div className="flex items-center justify-between px-5 sm:px-6 py-3.5 sm:py-4 border-b border-white/5 shrink-0">
        <div>
          <h2 className="text-sm font-mono tracking-[0.2em] text-slate-400 uppercase">
            What-If Simulator
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Adjust variables manually to simulate flood conditions.
          </p>
        </div>
        {onClose && (
          <button
      ...
      <div className="p-6 flex-1 overflow-y-auto space-y-8 relative">
        {isLiveOsiris && (
          <div className="absolute inset-0 z-10 bg-[#0a0f18]/80 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center">
            ...
            <h3 className="text-lg font-bold text-rose-400 font-mono tracking-widest mb-2">
              OSIRIS LIVE ACTIVE
            </h3>
            <p className="text-sm text-slate-300">
              The What-If Simulator is disabled while the system is locked to live satellite
              telemetry.
            </p>
            <p className="text-xs text-slate-500 mt-4 font-mono">
              Turn off OSIRIS in the top header to enable manual simulations.
            </p>
          </div>
        )}
        ...
        {/* RISK SCORE DISPLAY */}
        <div
          className={`rounded-2xl border p-4 flex items-center justify-between transition-all duration-500 ${riskBg}`}
        >
