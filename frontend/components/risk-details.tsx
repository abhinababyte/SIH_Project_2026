...
import type { Sensor, Severity } from "@/lib/flood-data";
    ...
    <div className="flex flex-col gap-6" aria-label="Risk Details">
      ...
      {selectedSensor ? (
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 flex flex-col gap-3">
            <div className="flex items-center gap-2 text-slate-400">
              {selectedSensor.type === "rain" ? (
                <Droplets className="size-4" />
              ) : selectedSensor.type === "river" ? (
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-bold text-slate-200">
                {selectedSensor.level.toFixed(1)}
              </span>
              ...
              <div key={feature} className="flex flex-col gap-1">
                ...
                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden flex">
                  {impact > 0 ? (
                    ...
                    <div
                      className="h-full bg-emerald-500/70 ml-auto"
                      style={{
                        width: Math.min(Math.abs(impact) * 100, 100) + "%",
                      }}
                    />
