...
const FloodMap = dynamic(() => import("@/components/flood-map"), {
  ssr: false,
  loading: () => (
    <div className="absolute inset-0 bg-slate-950 flex items-center justify-center text-slate-500 font-mono text-xs">
...
export default function ResidentDashboard() {
  ...
  >([{ id: "1", lat: 30.738, lng: 78.595, title: "Fallen Tree on River Road" }]);
      ...
      async (pos) => {
        ...
        try {
          const res = await fetch(
            `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=precipitation,soil_moisture_0_to_7cm`,
          );
  ...
  const compositeIndex = Math.min(
    100,
    Math.max(0, Math.round(rain * 1.2 + soil * 0.4 + river * 4)),
  );
  ...
  const getBgStyle = (sev: "safe" | "watch" | "danger" | string) => {
    switch (sev) {
      case "danger":
        return "bg-rose-500/10 border-rose-500/30 shadow-[0_0_20px_rgba(244,63,94,0.2)]";
      case "watch":
        return "bg-amber-500/10 border-amber-500/30 shadow-[0_0_20px_rgba(245,158,11,0.2)]";
      case "safe":
        return "bg-emerald-500/10 border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.2)]";
      default:
        return "bg-[#121923] border-white/5";
    }
  };
        ...
        <div className="min-h-screen bg-black text-white font-sans flex flex-col p-6 animate-in fade-in duration-500 relative z-[100]">
          <div className="flex-1 max-w-md mx-auto w-full flex flex-col justify-center space-y-8">
            ...
            <div className="bg-white/10 p-6 rounded-2xl border border-white/20">
              <h3 className="text-sm font-bold text-slate-400 mb-4 uppercase tracking-widest text-center">
                Safest Route
              </h3>
            ...
            <a
              href="sms:108?body=SOS! Flash Flood. Location: Lat 27.04, Long 88.26. 4 People."
            ...
            <button
              ...
              className="text-slate-500 text-xs underline pt-4 text-center w-full"
            >
      ...
      `,
        ...
        <div className="fixed top-16 left-0 right-0 h-8 z-40 bg-[#0E1626] border-b border-rose-500/20 flex items-center overflow-hidden pointer-events-none shadow-md">
          ...
          <div className="animate-marquee h-full text-[10px] font-mono tracking-widest text-rose-300 uppercase gap-12 px-6">
            <span className="flex items-center gap-2 h-full">
              ...
              <strong>HEAVY RAINFALL WARNING:</strong> 120mm expected in next 4 hours across Lower
              Basin.
            </span>
            <span className="opacity-30 h-full flex items-center">|</span>
            <span className="flex items-center gap-2 h-full">
              <Mountain className="size-3 text-orange-400 shrink-0" />
              <strong>SOIL INSTABILITY DETECTED:</strong> High risk of landslides in Sector 4
              (Millbrook Slope). Avoid area.
            </span>
            <span className="opacity-30 h-full flex items-center">|</span>
            <span className="flex items-center gap-2 h-full">
        ...
        <section className="relative px-4 sm:px-6 pt-6 sm:pt-8 pb-8 sm:pb-12 max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12">
          ...
          <div className="flex-1 space-y-6 sm:space-y-8 z-10">
            ...
            <h1 className="text-3xl sm:text-5xl md:text-7xl font-serif text-white tracking-tight leading-[1.1]">
              Your Community's <span className="italic text-slate-400">Shield</span>
              <br />
              Against the Storm.
            </h1>

            <p className="text-base sm:text-lg text-slate-400 max-w-xl leading-relaxed">
              Real-time flood alerts, active evacuation routes, and immediate shelter guidance
              tailored specifically to your village and elevation.
            </p>
            ...
            <div className="flex flex-wrap lg:flex-nowrap items-center gap-3 pt-2 sm:pt-4">
              <button
                onClick={() =>
                  document
                    .getElementById("shelters-section")
            ...
            <div className="flex items-start justify-between">
              <div className="w-full">
                {userLocation ? (
                  <>
                    <span
                      className={`inline-block px-3 py-1 text-xs font-bold tracking-widest uppercase rounded-full mb-3 border ${compositeSev === "danger" ? "bg-rose-500/20 text-rose-400 border-rose-500/30" : compositeSev === "watch" ? "bg-amber-500/20 text-amber-400 border-amber-500/30" : "bg-emerald-500/20 text-emerald-400 border-emerald-500/30"}`}
                    >
                      {compositeSev === "danger"
                        ? "CRITICAL DANGER"
                        : compositeSev === "watch"
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mt-1 mb-3">
                      ...
                      <strong className="text-white">
                        {locationName || "Your exact coordinates"}
                      </strong>
                    </p>
                  </>
                ) : (
                  <>
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed mb-4">
                      Showing generalized regional data.
                    </p>
            ...
            {userLocation ? (
              <>
                ...
                <div className="bg-black/30 rounded-2xl border border-white/5 p-4 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-full h-1 bg-white/5">
                    <div
                      className={`h-full ${compositeSev === "danger" ? "bg-rose-500 w-[80%]" : compositeSev === "watch" ? "bg-amber-500 w-[50%]" : "bg-emerald-500 w-[10%]"} animate-pulse`}
                    ></div>
                  </div>
                  ...
                  <div className="flex items-end justify-between mb-3 mt-1">
                    <div>
                      ...
                      {compositeSev === "danger" ? (
                        <div className="text-3xl font-black text-rose-400">
                          12
                          <span className="text-sm text-rose-400/70 ml-1 font-sans">mins left</span>
                        </div>
                      ) : compositeSev === "watch" ? (
                        <div className="text-3xl font-black text-amber-400">
                          45
                          <span className="text-sm text-amber-400/70 ml-1 font-sans">
                            mins left
                          </span>
                        </div>
                      ) : (
                    ...
                    <div className="text-right">
                      ...
                      {compositeSev === "danger" ? (
                        ...
                        <div className="text-sm font-bold text-amber-400 uppercase tracking-wider animate-pulse">
                          Prepare Kits
                        </div>
                      ) : (
                  ...
                  <div className="space-y-2 mt-4">
                    {compositeSev !== "safe" ? (
                      <>
                        <div className="flex items-center justify-between bg-white/5 p-2 rounded-lg border border-white/5">
                          <span className="text-xs text-slate-300 flex items-center gap-2">
                            <div className="size-1.5 rounded-full bg-amber-500"></div> Secure
                            Documents
                          </span>
                      ...
                      <div className="flex items-center justify-between bg-white/5 p-2 rounded-lg border border-white/5">
                        <span className="text-xs text-slate-300 flex items-center gap-2">
                          <div className="size-1.5 rounded-full bg-emerald-500"></div> All Clear
                        </span>
                ...
                <div className="bg-black/20 rounded-xl p-3 border border-white/5">
                  ...
                  <ul className="space-y-1.5">
                    <li className="text-[11px] text-slate-400 flex items-center gap-2">
                      <CloudRain className="size-3 text-[#5E6AD2]" /> {Math.round(rain)}mm rainfall
                      detected locally
                    </li>
                    <li className="text-[11px] text-slate-400 flex items-center gap-2">
                      <Droplets className="size-3 text-emerald-400" /> Local soil saturation at{" "}
                      {Math.round(soil)}%
                    </li>
                  </ul>
                </div>
              ...
              <div className="bg-black/20 rounded-xl p-6 border border-white/5 flex flex-col items-center justify-center text-center mt-2">
                ...
                <p className="text-xs text-slate-500 mt-1 max-w-[280px]">
                  To see precise evacuation windows and hyper-local sensor data, please share your
                  location.
                </p>
              </div>
            )}
        ...
        <section className="px-6 py-12 w-full max-w-7xl mx-auto border-t border-white/5">
          <div className="mb-8 w-full text-left">
            <div className="text-emerald-400 font-mono text-xs tracking-widest mb-2 uppercase">
              01 - Live Sensors
            </div>
            <h2 className="text-3xl md:text-4xl font-serif text-white">Regional readings</h2>
            <p className="text-slate-400 mt-2">
              Live telemetry from remote rainfall gauges and soil-moisture probes. Updated every 10
              seconds.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div
              className={`border rounded-2xl sm:rounded-3xl p-5 sm:p-8 flex flex-col items-center justify-center relative overflow-hidden transition-colors duration-1000 ${getBgStyle(compositeSev)}`}
            >
              ...
              <div className="text-sm text-slate-400 font-mono tracking-widest font-bold uppercase mb-1 z-10">
                Composite Risk Index
              </div>
              <div className="text-[10px] text-slate-500 font-mono tracking-widest uppercase mb-4 sm:mb-6 z-10 opacity-70 bg-black/20 px-2 py-1 rounded">
                LOC: 30°42'N 78°27'E
              </div>
              ...
              <div className="relative size-40 sm:size-48 flex items-center justify-center z-10">
                ...
                >
                  ...
                  <circle
                    ...
                    strokeWidth="12"
                    strokeDasharray="502"
                    strokeDashoffset={502 - 502 * (compositeIndex / 100)}
                    strokeLinecap="round"
                    className={`transition-all duration-1000 ${getTextColor(compositeSev)}`}
                  />
                </svg>
                <div className="text-center">
                  <div className={`text-4xl sm:text-5xl font-serif ${getTextColor(compositeSev)}`}>
                    {compositeIndex}
                  </div>
              ...
              <div className="mt-6 sm:mt-8 text-sm text-slate-400 z-10 text-center">
                ...
                <span className="text-white font-bold">
                  {Math.min(100, Math.round(rain * 2.5))}%
                </span>
            ...
            <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div
                className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl border flex flex-col justify-between transition-colors duration-1000 ${getBgStyle(rainSev)}`}
              >
                <div className="flex items-start justify-between">
                  <div
                    className={`text-xs sm:text-sm font-bold font-mono tracking-widest uppercase ${getTextColor(rainSev)} opacity-80`}
                  >
                    Rainfall
                  </div>
                ...
                <div className="mt-4 sm:mt-8">
                  <div className={`text-3xl sm:text-4xl font-serif ${getTextColor(rainSev)}`}>
                    {Number(rain).toFixed(2)}{" "}
                    <span className="text-xs sm:text-sm font-sans opacity-70">mm/hr</span>
                  </div>
                  <p
                    className={`mt-2 sm:mt-3 text-xs leading-relaxed hidden sm:block ${getTextColor(rainSev)} opacity-70`}
                  >
                    {rainSev === "danger"
                      ? "Extreme downpour. Ground absorption halted. Immediate flash flood risk."
                      : rainSev === "watch"
                        ? "Steady downpour. Ground absorption is slowing down. Expected to continue."
                        : "Light rain. Natural drainage systems are functioning normally."}
                  </p>
                </div>
              </div>

              <div
                className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl border flex flex-col justify-between transition-colors duration-1000 ${getBgStyle(soilSev)}`}
              >
                <div className="flex items-start justify-between">
                  <div
                    className={`text-xs sm:text-sm font-bold font-mono tracking-widest uppercase ${getTextColor(soilSev)} opacity-80`}
                  >
                    Soil Moisture
                  </div>
                ...
                <div className="mt-4 sm:mt-8">
                  <div className={`text-3xl sm:text-4xl font-serif ${getTextColor(soilSev)}`}>
                    {Number(soil).toFixed(2)}
                    <span className="text-lg sm:text-xl font-sans ml-1 opacity-70">%</span>
                  </div>
                  <p
                    className={`mt-2 sm:mt-3 text-xs leading-relaxed hidden sm:block ${getTextColor(soilSev)} opacity-70`}
                  >
                    {soilSev === "danger"
                      ? "Critical saturation. Mudslides likely in steep areas. Avoid slopes."
                      : soilSev === "watch"
                        ? "Ground is heavily saturated. Approaching critical threshold for potential landslides."
                        : "Optimal soil absorption levels. No immediate landslide threat."}
                  </p>
                </div>
              </div>

              <div
                className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl border flex flex-col justify-between transition-colors duration-1000 ${getBgStyle(slopeSev)}`}
              >
                <div className="flex items-start justify-between">
                  <div
                    className={`text-xs sm:text-sm font-bold font-mono tracking-widest uppercase ${getTextColor(slopeSev)} opacity-80`}
                  >
                    Slope Stability
                  </div>
                ...
                <div className="mt-4 sm:mt-8">
                  <div className={`text-3xl sm:text-4xl font-serif ${getTextColor(slopeSev)}`}>
                    {slopeStability}
                    <span className="text-xs sm:text-sm font-sans ml-1 opacity-70">/ 100</span>
                  </div>
                  <p
                    className={`mt-2 sm:mt-3 text-xs leading-relaxed hidden sm:block ${getTextColor(slopeSev)} opacity-70`}
                  >
                    {slopeSev === "danger"
                      ? "Critical instability. Active soil displacement detected in sectors 4 and 7."
                      : slopeSev === "watch"
                        ? "Moderate risk. Minor soil stress detected. Actively monitoring."
                        : "Stable bedrock and soil structure. No displacement detected."}
                  </p>
                </div>
              </div>

              <div
                className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl border flex flex-col justify-between transition-colors duration-1000 ${getBgStyle(leadSev)}`}
              >
                <div className="flex items-start justify-between">
                  <div
                    className={`text-xs sm:text-sm font-bold font-mono tracking-widest uppercase ${getTextColor(leadSev)} opacity-80`}
                  >
                    Lead Time
                  </div>
                ...
                <div className="mt-4 sm:mt-8">
                  <div className={`text-3xl sm:text-4xl font-serif ${getTextColor(leadSev)}`}>
                    {leadTime}{" "}
                    <span className="text-xs sm:text-sm font-sans opacity-70">minutes</span>
                  </div>
                  <p
                    className={`mt-2 sm:mt-3 text-xs leading-relaxed hidden sm:block ${getTextColor(leadSev)} opacity-70`}
                  >
                    {leadSev === "danger"
                      ? "Imminent threat window. Prepare for immediate evacuation of low-lying areas."
                      : leadSev === "watch"
                        ? "Estimated window before critical flood levels are reached at the primary river basin."
                        : "Sufficient time to secure property if conditions worsen."}
                  </p>
                </div>
              </div>
            </div>
        ...
        <section
          ...
          className="px-6 py-12 w-full max-w-7xl mx-auto border-t border-white/5 scroll-mt-24"
        >
          <div className="mb-8 w-full text-left">
            <div className="text-emerald-400 font-mono text-xs tracking-widest mb-2 uppercase">
              02 - Evacuation Routing
            </div>
            <h2 className="text-3xl md:text-4xl font-serif text-white">Landmark Navigation</h2>
            <p className="text-slate-400 mt-2">
              GPS drifts in heavy rain. Follow verified local landmarks to the nearest shelter.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Link
              href="/resident/map"
              className="bg-[#121923] border border-white/5 rounded-3xl p-2 relative overflow-hidden group block min-h-[400px]"
            >
            ...
            <div className="bg-[#121923] border border-white/5 rounded-3xl p-8 shadow-xl">
              ...
              <div className="relative border-l border-white/10 ml-4 space-y-8 pb-4">
                <div className="relative pl-6">
                  ...
                  <p className="text-sm text-slate-400">
                    Head north uphill towards the main village square.
                  </p>
                </div>
                ...
                <div className="relative pl-6">
                  ...
                  <div className="bg-black/30 rounded-xl p-3 border border-white/5 mt-2 flex gap-3 items-center">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed">
                      Turn LEFT immediately after passing the large peepal tree.{" "}
                      <strong className="text-rose-400">DO NOT take the lower dirt path.</strong>
                    </p>
                  </div>
                </div>
                ...
                <div className="relative pl-6">
                  ...
                  <div className="text-sm text-slate-300">
                    Enter through the main blue gates. Check in with the volunteer immediately to
                    sync your family status.
                  </div>
                </div>
              </div>
              ...
              {/* Hidden inline form */}
              <div
                id="route-report-form"
                className="hidden mt-4 p-4 bg-black/40 border border-rose-500/30 rounded-2xl transition-all"
              >
                <h4 className="text-rose-400 font-bold text-sm mb-3">
                  Report Blockage on this Route
                </h4>
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                ...
                >
                  ...
                  <textarea
                    rows={2}
                    placeholder="What is blocking the path? (e.g. fallen tree, deep water)..."
                    className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-sm text-white mb-3 focus:outline-none focus:border-rose-500 resize-none"
                  ></textarea>
                  <button
                    type="submit"
                    className="w-full py-3 bg-rose-500 hover:bg-rose-600 text-slate-950 font-bold rounded-xl transition-colors"
                  >
                    Submit Route Report
                  </button>
                </form>
        ...
        >
          <div className="mb-8 w-full text-left">
            ...
            <p className="text-slate-400 mt-2">
              Real-time capacity tracking for verified community shelters and registered safe
              houses.
            </p>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-[#121923] border border-emerald-500/30 rounded-2xl p-6 relative overflow-hidden flex flex-col md:flex-row gap-6 items-center shadow-lg">
                ...
                <div className="flex-1 w-full">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-5 border-y border-white/5 py-3 sm:py-4">
                    ...
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-rose-500/70 mb-1 font-mono">
                        Injured
                      </div>
                ...
                <div className="w-full md:w-auto shrink-0 flex flex-col gap-2">
                  <Link
                    href="/resident/map"
                    className="w-full px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold rounded-xl transition-colors flex items-center justify-center gap-2"
                  >
                    <Navigation className="size-4" />
                    Route
                  </Link>
              ...
              <div className="bg-[#121923] border border-amber-500/30 rounded-2xl p-6 relative overflow-hidden flex flex-col md:flex-row gap-6 items-center shadow-lg">
                ...
                <div className="w-full md:w-auto shrink-0 flex flex-col gap-2">
                  <Link
                    href="/resident/map"
                    className="w-full px-6 py-3 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 font-bold rounded-xl transition-colors flex items-center justify-center gap-2"
                  >
                    <Navigation className="size-4" />
                    Route
                  </Link>
        ...
        >
          <div className="mb-8 w-full text-left">
            <div className="text-emerald-400 font-mono text-xs tracking-widest mb-2 uppercase">
              04 - Crowdsourced Intel
            </div>
            <h2 className="text-3xl md:text-4xl font-serif text-white">
              Community Incident Reports
            </h2>
            <p className="text-slate-400 mt-2">
              Live reports from your neighbors. Ground-truth data is critical when sensors fail.
            </p>
          </div>
        ...
        <footer className="w-full bg-[#0a0f18]/95 backdrop-blur-xl border-t border-white/5 px-4 sm:px-6 lg:px-12 py-5 sm:py-6 flex flex-col sm:flex-row items-center justify-between gap-4 mt-12 z-10 shadow-[0_-4px_24px_rgba(0,0,0,0.5)] safe-bottom">
          ...
          <div className="text-[10px] font-mono tracking-wider text-slate-500 flex items-center">
            <span className="flex items-center gap-1.5 text-slate-400 text-center">
              Created By {" "}
              <strong className="text-emerald-500 font-bold tracking-widest">Codex Gigas</strong>
            </span>
          </div>
        </footer>
      </div>
